...
public class LinearRegression {
    private final static String TAG = LinearRegression.class.getSimpleName();
    ...
    public boolean regress(double[] Y, double[][] X, double[] W)  {
        ...
        if (debug) Log.v(TAG, "V: " + Arrays.deepToString(V));
        if (!invertSymmetricMatrix(V))  {
            return false;
        }
        if (debug) Log.v(TAG, "Inv(V): " + Arrays.deepToString(V));
    ...
    public boolean invertSymmetricMatrix(double[][] V)    {
        ...
        for (M = 0; M < N; M++)  {
            ...
            if (Big == 0) {
                if (debug) Log.v(TAG, "Max ABS number is zero!");
                return false;
            }
